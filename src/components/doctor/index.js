...
class Doctors extends Component {
  ...
  componentDidMount() {
    database.ref('codes').on('value', data => {
      ...
      list = list.filter(item => item.type === 'doctor')
      this.setState({ list })
    })
  }
    ...
    database
      ...
      .once('value', data => {
        ...
        if (!data) {
          ...
          this.props.alert.success('Doctor Added successfully')
        } else {
  ...
  render() {
    return (
      <div>
        <h1 className="labels">Doctors</h1>
        {this._renderList()}
      </div>
    )
  }
...
export default withAlert(Doctors)
